    ...
    function getWinner(userChoice, computerChoice) {
        switch (userChoice) {
          case computerChoice:
            return 'Empate';
          case 'rock':
            return computerChoice === 'paper' ? 'Perdiste' : 'Ganaste';
          case 'paper':
            return computerChoice === 'scissors' ? 'Perdiste' : 'Ganaste';
          case 'scissors':
            return computerChoice === 'rock' ? 'Perdiste' : 'Ganaste';
          
        }
      }

const imagenRock = "./images/icon-rock.svg"
...
const winnerTitle = document.querySelector("#option-result p")
...
function saveLocalStoreageScore(winnerTitle){
  const prevScore = Number(window.localStorage.getItem(`score`))

  if (winnerTitle === 'Ganaste') {
...
function  showWinner(winner, computerChoice, userChoice){
    tablero.classList.add('hidden')
    resultados.classList.remove('hidden')
    spanImagenUser.src = imagenOptions[userChoice]
    spanImagenCPU.src = imagenOptions[computerChoice]
    spanColorCpu.classList.add(colorsOptions[computerChoice])
    spanColorUser.classList.add(colorsOptions[userChoice])
    winnerTitle.innerHTML = `<h1>${winner}</h1>`
    saveLocalStoreageScore(winner)
...
useroptions.forEach(Option => {
    Option.addEventListener('click', function(){
        ...
        //generar la lógica para saber quien ganó
        const winner = getWinner(user, cpu)
        //ocultar el tablero de juego
        showWinner(winner, cpu, user)
        
        

        // mostrar el tablero de resultados
        // mostrar el tablero de resultados
